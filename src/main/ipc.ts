...
export function registerIpcHandlers(): void {
  ...
  ipcMain.handle('git:amend', wrap((path: string, message: string) => git.amendCommit(path, message)))
  ipcMain.handle('git:undoLast', wrap((path: string) => git.undoLastCommit(path)))
  ipcMain.handle('git:lastMessage', wrap((path: string) => git.lastCommitMessage(path)))
